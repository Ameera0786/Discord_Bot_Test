const { adjustStat} = require('./userManager');

// Food definitions
const allFoods = {
    apple: { hunger: 1, emoji: '🍎' },
    sandwich: { hunger: 5, emoji: '🥪' },
    steak: { hunger: 10, energy: 5, emoji: '🥩' },
    coffee: { hunger: 0, energy: 15, emoji: '☕' },
    orange: { hunger: 3, luck: 0.5, emoji: '🍊', temporary: true, duration: 60_000 }
};
...
function addFood(user, foodName, foodData) {
    const inventory = getInventory(user);

    if (inventory[foodName]) { inventory[foodName].qty += 1; }
    else { inventory[foodName] = { ...foodData, qty: 1 }; }
}

module.exports = { getInventory, eatFood, addFood, resolveFood };